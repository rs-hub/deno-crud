function buildWhere(data: object, lengthValue = 1) {
    ...
    const conditions = Object.entries(data).reduce((prev, [value, key], i) => {
        values.push(key);
        return prev ? `${prev} AND ${`${value} = $${lengthValue + i}`}` : `${value} = $${lengthValue + 1}`;
    }, "");
    return {
        conditions,
        valuesWhere: values,
...
export const buildUpdateQuery = ({ tableName, where, data }: { tableName:string, where:object, data:object}) => {
    const set = [];
    let values = [];
